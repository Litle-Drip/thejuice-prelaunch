...
<body class="min-h-screen">
  <main class="max-w-5xl mx-auto px-6 py-12 space-y-12">
    ...
    <section class="glass rounded-2xl p-8 space-y-10">
      ...
      <p id="voteStatus" class="text-xs text-center text-slate-400 min-h-[1.25rem]" aria-live="polite">Local mode: totals are per-device only.</p>
      ...
      <!-- Donate to Treasury (HTML must be OUTSIDE any <script>) -->
      <div class="glass rounded-2xl p-5 mt-6">
        <div class="flex items-start gap-4 flex-col md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <div class="mt-2">
              <code id="treasuryText" class="text-xs bg-white/5 border border-white/10 rounded px-2 py-1 break-all"></code>
            </div>
            <div class="flex gap-2 mt-3">
              <button id="copyTreasury" class="btn rounded-lg px-3 py-2 bg-white/10 border border-white/20 hover:bg-white/15">Copy address</button>
              <a id="openInWallet" class="btn rounded-lg px-3 py-2 bg-cyan-500/80 hover:bg-cyan-500 text-white" href="#" rel="noopener">Open in wallet</a>
              <!-- Optional preset -->
              <!-- <a class="btn rounded-lg px-3 py-2 bg-white/10 border border-white/20 hover:bg-white/15" href="ethereum:0x5a0d1c4b58dbea38471841053de97cb158f19a6e?value=10000000000000000">Donate 0.01 ETH</a> -->
            </div>
            <p class="text-xs text-slate-500 mt-2">Tip: most wallets accept the QR below or pasting the address.</p>
          </div>
          <div class="mt-4 md:mt-0">
            <canvas id="qrDonate" width="160" height="160" class="rounded-lg border border-white/20 bg-white/5"></canvas>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', ()=>{
      ...
      if (aWallet) {
        aWallet.href = ethUri;
        aWallet.setAttribute('target', '_self');
        aWallet.setAttribute('rel', 'noopener');
        const short = `${addr.slice(0,6)}…${addr.slice(-4)}`;
        aWallet.textContent = `Open in wallet (${short})`;
      }
